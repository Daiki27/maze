    ...
    override func viewDidLoad() {
        ...
        for y in 0 ..< maze.count {
            for x in 0 ..< maze[y].count {
                switch maze[y][x]{
                    case 1:
                    let wallView = createView(x:x,y:y, width: cellWidth, height: cellHeight, offsetX: cellOffsetY,offsetY:cellOffsetY)
        ...
        let handler: CMAccelerometerHandler = {(CMAccelerometerData:CMAccelerometerData?, error:Error?) ->
            ...
            for wallRect in self.wallRectArray {
                ...
                if (wallRect.intersects(self.playerView.frame)){
                    print("Game Over")
                    self.gameCheck(result:"Game Over", messeage: "壁に当たりました。")
                    return
                }
            }
